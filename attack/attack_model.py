...
class AttackModel:
    ...
    def llm_eval(self, model, data_loader, cfg, idx_rate, perturb_fn=None, refer_model=None):
        ...
        for iteration, texts in enumerate(data_loader):
            logger.info(f"[llm_eval] Iteration {iteration}, batch size: {len(texts['text'])}")
        ...
        logger.info(f"[llm_eval] Completed iteration {iteration}")
    ...
    def eval_perturb(self, model, dataset, cfg):
        ...
        for i in tqdm(range(0, cfg["perturbation_number"])):
            ...
            logger.info(f"[eval_perturb] Starting perturbation loop i={i}, idx_rate={idx_rate}")
    ...
    def data_prepare(self, kind, cfg):
        ...
        data_path = os.path.join(
            PATH,
            cfg["attack_data_path"],
            f"attack_data_{cfg['safe_model_name']}@{cfg['safe_dataset_name']}"
        )
        ...
        if not utils.check_files_exist(*pathlist) or not cfg["load_attack_data"]:
            ...
            attack_dir = os.path.join(data_path, kind)
            ...
            if not os.path.exists(mem_path):
                ...
                logger.info("Loading cached member features...")
            ...
            if not os.path.exists(nonmem_path):
                ...
                logger.info("Loading cached non-member features...")
    ...
    def feat_prepare(self, info_dict, cfg):
        ...
        if cfg["attack_kind"] == "stat":
            ...
            feat = - np.concatenate([mem_feat, nonmem_feat], axis=0).mean(axis=tuple(range(1, mem_feat.ndim)))
    ...
    def sentence_perturbation(self, texts, idx_rate):
        ...
        if '' in perturbed_texts:
            print(f"⚠️ Max attempts reached. Falling back to original texts for {perturbed_texts.count('')} items.")
            for i, x in enumerate(perturbed_texts):
                if x == '':
                    perturbed_texts[i] = texts[i]  # revert to original
